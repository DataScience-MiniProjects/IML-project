...
pred_dummy <- c()
for (i in 1:nrow(npf_test)) {
  r <- runif(1)
  if (r <= 0.5) {
    pred_dummy[i] <- "nonevent"
  } else if (r <= 258/458) {
    pred_dummy[i] <- "Ia"
  } else if (r <= 341/458) {
    pred_dummy[i] <- "Ib"
...
conf <- confusionMatrix(factor(pred_dummy), factor(test.pc$class4))

perp <- function(probs, y=npf_test$class4) {
  exp(-mean(log(ifelse(y!='nonevent', probs, 1-probs))))
}

pd2 <- perp(rep(0, 965))
pd1 <- perp(ifelse(pred_dummy!='nonevent', 1, 0))
p <- perp((probs_test4$Ia+probs_test4$Ib+probs_test4$II))
